...
void generate_report(pt::ptree &root, std::string project) {
  ...
  output << "<!DOCTYPE html>\n"
      ...
      "  title.innerHTML = json[\"project\"] + \" analysis results\";\n"
      "  info.appendChild(document.createTextNode(\"Project \" + json[\"project\"] + \" (version \" + json[\"version\"] + \") total size is \" +\n"
      "                                            (json['totalsize'] / 1024).toFixed(2) + \" kilobytes\"));\n"
      ...
      "\n"
      "  var stats = document.createElement(\"ul\");     //Is this good?\n"
      ...
      "\n"
      "  /*var sizes = history;\n"
      "  for (var hiskey in history) {\n"
      "    var current = history[hiskey];\n"
      "    var element = document.createElement(\"li\");\n"
      "    element.innerHTML = current.size + \" bytes\";\n"
      "    list.appendChild(element);\n"
      "  }\n"
      "  console.log(sizes);\n"
      ...
      "    }\n"
      "\n"
      "    //drawing the grid lines (not needed if using 2nd bar style)\n"
      "  /*  this.options.gridScale = Math.round((maxValue - minValue) / 10);\n"
      "\n"
      "    var gridValue = 0;\n"
      "    while (gridValue <= maxValue) {\n"
      "      var gridY = (this.canvas.height * (1 - gridValue / maxValue)) - 30;\n"
      "      drawLine(\n"
      "        this.ctx,\n"
      "        0,\n"
      "        gridY,\n"
      ...
      "\n"
      "    fillBackground(this.ctx, this.canvas.width, bar_area_height);     //Fill bar area with a gray background\n"
      "    for (element in this.options.data) {\n"
      "      var value = this.options.data[element].size;\n"
      "      var version = this.options.data[element].version;\n"
      "\n"
      "      // 1st - height relative to max value only\n"
